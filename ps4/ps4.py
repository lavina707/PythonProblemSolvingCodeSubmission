...
def build_coder(shift):
    ...
    for i in letters: 
        cipher_number = (letters.index(i)+shift)%27 
        cipher_letter = letters[cipher_number] 
        coder[i]=cipher_letter 
        if i != ' ': 
            coder[i.upper()]=cipher_letter.upper() 
    return coder
#print(build_coder(3))
...
def apply_coder(text, coder):
    ...
    for character in text: 
        if character in coder: 
            cipher_text += coder[character] 
        else: 
            cipher_text += character
    return cipher_text 

#print(apply_coder("Hello, world!", build_encoder(3)))  
#print(apply_coder("Hello, world!", build_decoder(3)))
...
def find_best_shift(wordlist, text):
    ...
    for shift in range(28): 
        ...
        decrypted_words = decoded_text.split() 
        valid_words = 0 
...
def apply_shifts(text, shifts):
    ...
    for tup in shifts: 
        first = text[:tup[0]]         
        second = text[tup[0]:] 
        text = first + apply_shift(second, tup[1])
    return text
